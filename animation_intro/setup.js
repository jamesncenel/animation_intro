...
var lowerArm2Color = vec4(1.0, 0.0, 0.0, 1.0);
...
var theta = [0, 0, 0, 0, 0, 0, 180, 0, 180, 0, 90];
//frame 1: var theta = [0, 0, 0, 0, 0, 0, 180, 0, 180, 0, 90];
//frame 2: var theta = [0, 0, 0, 0, 0, 0, 180, 0, 180, 0, 90];
//order: torso, head, upperarm1, lowerarm1, upperarm2, lowerarm2, upperleg1, lowerleg1, upperleg2, lowerleg2
...
//Viewer and perspective objects:
var viewer = 
{
...
function initNodes(Id) {
    ...
    m = translate(0.0, keyFrameTime, 0.0);
    ...
    m = mult(m, rotate( keyFrameTime* 30, 0, 0, 1));
...
function leftUpperArm() {
    ...
    //instanceMatrix = mult(instanceMatrix, rotate( keyFrameTime* 30, 0, 0, 1));
...
function leftLowerArm() {
    ...
    for(var i =0; i<6; i++) gl.drawArrays(gl.TRIANGLE_FAN, 4*i, 4);
    //each iteration creates a face of the object, that's why it is 8
}
   ...
   // configureTexture();
    ...
    document.getElementById("gl-canvas").onmousemove = function (event)
    {
        ...
        if(makeChange == 1) {
            ...
            // Recompute the view
            console.log(lookAt(vec3(viewer.eye), viewer.at, viewer.up))
...
var render = function() {
        ...
        //Altered Texture for the ground
        texture2 = gl.createTexture();
        gl.bindTexture( gl.TEXTURE_2D, texture2 );
        gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, texSize, texSize, 0, gl.RGBA, gl.UNSIGNED_BYTE, image2);
        ...
        instanceMatrix = mult(modelViewMatrix, translate(0.0, -5.3, 0.0) );
        instanceMatrix = mult(instanceMatrix, scale4( 15.0, 1.0, 15.0));
        gl.uniformMatrix4fv(modelViewMatrixLoc, false, flatten(instanceMatrix));
        gl.uniform4fv(colorLoc, flatten(torsoColor) );
